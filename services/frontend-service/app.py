...
async def items(request: Request):
    async with httpx.AsyncClient(timeout=10) as client:
        ...
        r.raise_for_status()
    return templates.TemplateResponse("items.html", {"request": request, "items": r.json()})

@app.get("/checkout/{item_id}", response_class=HTMLResponse)
async def checkout_page(request: Request, item_id: int):
    async with httpx.AsyncClient(timeout=10) as client:
        r = await client.get(f"{ANIME_SERVICE_URL}/items/{item_id}")
        r.raise_for_status()
    return templates.TemplateResponse("checkout.html", {"request": request, "item": r.json()})
...
async def checkout(
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        item_response = await client.get(f"{ANIME_SERVICE_URL}/items/{item_id}")
    ...
    return templates.TemplateResponse(
        "order.html",
        {"request": request, "order": order, "message": result["message"]}
    )

@app.get("/health")
async def health():
    return {"status": "UP", "service": "frontend-service"}
